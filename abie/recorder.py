...
class SimulationDataRecorder(object):
    ...
    def record(self, state_dict):
        ...
        if self._monitored_quantities is None:
            quantities = ['a', 'ecc', 'inc', 'x', 'y', 'z', 'vx', 'vy', 'vz', 'mass', 'time', 'energy']
        else:
        ...
        for i, qty in enumerate(quantities):
            if qty in state_dict:
                d = state_dict[qty]
                if d.ndim == 2:
                    d = d[~np.isnan(d).any(axis=1)] # remove the parts of buffer that contains NaN
                else:
                    d = d[~np.isnan(d)]

                if qty in self.data and self.data[qty] is not None:
                    # append
                    indices = np.where(np.in1d(self._particles_hash_vec, state_dict['hash'][0]))[0] # the indices to be updated
                    if self._monitored_particles is not None:
                        if d.ndim == 2:
                            tmp_data = np.zeros((d.shape[0], self.data[qty].shape[1])) * np.nan
                            ...
                            self.data[qty] = np.append(self.data[qty], d[self._monitored_particles])
                    else:
                        if d.ndim == 2:
                            tmp_data = np.zeros((d.shape[0], self.data[qty].shape[1])) * np.nan
                            tmp_data[:, indices] = d
                    ...
                    # New record
                    self._particles_hash_vec = state_dict['hash'][0] # get the hash of all particles at t = 0
                    if self._monitored_particles is not None:
